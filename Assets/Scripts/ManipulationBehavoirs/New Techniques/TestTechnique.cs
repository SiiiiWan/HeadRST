...
public class TestTechnique : PositionRotationProvider
{
    ...
    public override Vector3 GetPositionOutput(Vector3 currentPosition)
    {
        ...
        float baseGain = VitLerp(Math.Abs(HeadData.HeadSpeed), 0, 0.8f, 0.1f, 0.6f);

        if (_currentMode == AH_ControlMode.Gaze)
        {
            currentPosition += gazeDirection * HeadData.DeltaHeadY * baseGain;
            currentPosition = gazeOrigin + gazeDirection * Mathf.Clamp(Vector3.Distance(gazeOrigin, currentPosition), 1, 20);
        ...
        {
            // currentPosition += directionFromGazeOrigin * HeadData.DeltaHeadY * baseGain * EdgeGain();
            // currentPosition = gazeOrigin + directionFromGazeOrigin * Mathf.Clamp(Vector3.Distance(gazeOrigin, currentPosition), 1, 20);
